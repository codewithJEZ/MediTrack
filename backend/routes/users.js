...
router.post('/', (req, res) => {
  ...
  ).run(name, username, hashed, role || 'staff');
...
router.put('/:id', (req, res) => {
  ...
  ).run(name, username, role, req.params.id);
  if (result.changes === 0) return res.status(404).json({ error: 'Not found' });
  res.json({ updated: true });
});

router.post('/:id/reset-password', (req, res) => {
  const result = db.prepare('UPDATE users SET password = ? WHERE id = ?').run(bcrypt.hashSync('123456', 10), req.params.id);
  if (result.changes === 0) return res.status(404).json({ error: 'Not found' });
  res.json({ reset: true });
});
